...
export default function isSrcPlayable(src) {
  ...
  const canplay = audio.canPlayType(type).replace(/no/, '')
  return Boolean(canplay)
}

export function createError(sources) {
  // All sources are unplayable
  const err = new Error('This browser does not support any of the following sources:\n    ' +
      sources.join(', ') + '\n' +
